...
function setup() {
  ...
  carrotTex = PIXI.Loader.shared.resources.bullet.texture;
  var grass = PIXI.Loader.shared.resources.grass.texture;
...
function updateView(object) {
  ...
  for (var i = 1; i < samples; i++) {
    vecs.push(
      vec.rotate(
        object.rotation - object.FOV / 2 + object.FOV * (i / (samples + 1))
      )
    );
  }
  ...
  if (levels[currentLevel] != undefined) {
    for (var structure of levels[currentLevel].structures) {
      ...
      for (var p = 0; p < structure.coords.length; p++) {
        ...
        if (
          vec.length() < object.viewDistance &&
          left.cross(vec) > 0 &&
          right.cross(vec) < 0
        ) {
          ...
          var direction =
            vec.cross(
              Vector.add(
                structure.pointNormals[p][0],
                structure.pointNormals[p][1]
              ).normalize()
            ) > 0;
          ...
          if (!nDirection1 || !nDirection2) {
            ...
            if (nDirection1 || nDirection2) {
              var nextEdge = vec
                .clone()
                .multiply(object.viewDistance / vec.length());
  ...
  vecs = vecs.sort(function (a, b) {
    ...
    if (Math.abs(angle) < 0.00000001) {
      if (a.edgeSide != undefined) return a.edgeSide;
      if (b.edgeSide != undefined) return b.edgeSide;
    }
    return angle;
  });
...
function evaluateControls() {
  var offsetX = 0;
  var offsetY = 0;
  if (keys["d"]) offsetX -= movementSpeed;
  if (keys["s"]) offsetY -= movementSpeed;
  if (keys["a"]) offsetX += movementSpeed;
  if (keys["w"]) offsetY += movementSpeed;
  if (keys[" "] && levels[currentLevel] != undefined) {
    for (var npc of levels[currentLevel].npcElements) {
      var distance = Vector.subtract(
        new Vector(player.position),
        new Vector(npc.position)
      ).length();
      if (distance < 30) {
        stage.removeChild(npc.triangle);
        stage.removeChild(npc);
        levels[currentLevel].npcElements.splice(
          levels[currentLevel].npcElements.indexOf(npc),
          1
        );
      }
    }
...
let text = new PIXI.Text("Escape from Russia", {
  fontFamily: "Arial",
  fontSize: 90,
  fill: 0xffffff,
  align: "left",
...
let tutorial = new PIXI.Text(
  "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\nUse WASD to move and press Space when near an enemy to take them out.\n Get to the Star to advance, and avoid enemy lines of sight.",
  { fontFamily: "Arial", fontSize: 35, fill: 0xffffff, align: "left" }
);
...
let winText = new PIXI.Text(
  "\n\n\n                          Congratulations! \n                           You Win",
  { fontFamily: "Arial", fontSize: 90, fill: 0xffffff, align: "center" }
);

function moveAlongPath(npc) {
  if (npc.path == undefined) return;

  if (!npc.currentVec) {
    if (npc.path[npc.pathIndex] != undefined) {
      npc.currentVec = npc.path[npc.pathIndex].clone();
    } else if (npc.pathIndex < npc.path.length * 2) {
      npc.currentVec = Vector.negative(
        npc.path[npc.path.length - (npc.pathIndex - npc.path.length) - 1]
      );
    } else {
      npc.pathIndex = 0;
      npc.currentVec = npc.path[npc.pathIndex].clone();
    ...
    for (var npc of levels[currentLevel].npcElements) {
      ...
      for (var i = 0; i < npc.triangle.path.length - 1; i += 2) {
        adjustedPath.push(
          Vector.add(
            npcPosition,
            new Vector(npc.triangle.path[i], npc.triangle.path[i + 1])
          )
        );
      }
      if (isPlayerWithin(adjustedPath)) loadLevel(currentLevel);
    }
